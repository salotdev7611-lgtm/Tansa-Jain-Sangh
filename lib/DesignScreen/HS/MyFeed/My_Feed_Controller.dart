...
class MyFeedController extends GetxController {
  ...
  RxList<Map<String,dynamic>> listOfMyFeed = <Map<String,dynamic>>[].obs;
  ...
  Future<void> myFeed() async{
    try{
      get.value = true;
      final response = await http.get(
        Uri.parse("${ApiUrl.baseUrl}${ApiUrl.post}&id=${loginScreenController.userId.value}"),
        headers: {
          "Content-Type": "application/json",
          "x-api-key": ApiUrl.xApikey,
          "Authorization": ApiUrl.token,
      ...
      if(response.statusCode == 200){
        final responseData = jsonDecode(response.body);
        if(responseData["success"] == true){
          listOfMyFeed.value = List<Map<String,dynamic>>.from(responseData["data"]);
          like.assignAll(
            listOfMyFeed.map((p) => p["has_liked"] == true).toList(),
          );
          // 🔖 bookmark state
          saveList.assignAll(
            listOfMyFeed.map((p) => p["has_bookmarked"] == true).toList(),
          );
          likeCount.assignAll(
            listOfMyFeed.map(
          ...
          print("listOfMyFeed ${listOfMyFeed.value}");
        }
        else{
          get.value = false;
